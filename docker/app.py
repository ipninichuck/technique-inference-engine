...
def train_model(model_name: str, params: TrainParams):
    ...
    hyperparams_to_fit = {
        ...
        "regularization_coefficient": params.regularization_coefficient,
    }
    if params.learning_rate is not None:
        hyperparams_to_fit["learning_rate"] = params.learning_rate
    if params.c is not None:
        hyperparams_to_fit["c"] = params.c

    try:
        mse = tie_engine.fit(**hyperparams_to_fit)
        state["trained_models"][model_name] = (tie_engine, hyperparams_to_fit)
        return {
            "message": f"Model '{model_name}' trained successfully.",
            "model_name": model_name,
            "mse": mse,
            "best_hyperparameters": hyperparams_to_fit,
        }
    except Exception as e:
